...
function All() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const resources = responseData.allUsers || [];
  ...
  // const bookResources = async (resource) => {
  //   try {
  //     const url = 'http://localhost:3000/user/borrowresources';
  //     const response = await fetch(url, {
  //       method: 'POST',
  //       headers: {
  //         'Content-type': 'application/json',
  //       },
  //       body:JSON.stringify(borrower,resource)
  //     });
  //     const responseData = await response.json();
  //     console.log(responseData);
  //     if (responseData.message) {
